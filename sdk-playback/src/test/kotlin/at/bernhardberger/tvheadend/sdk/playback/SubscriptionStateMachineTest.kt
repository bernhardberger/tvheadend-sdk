...
private class RecordingSubscriptionConnection : SubscriptionConnection {
...
private enum class Call { COLLECTION_REGISTERED, SUBSCRIBE, UNSUBSCRIBE, LIVE_COMMIT }
...
private fun successfulConfirmation(): SubscriptionOperationResult<SubscriptionConfirmation> =
    SubscriptionOperationResult.Ok(SubscriptionConfirmation(null, null, null, null))

private fun started(vararg streams: SubscriptionStream): SubscriptionEvent.Started =
...
private fun packet(payload: SubscriptionBinary = CountingBinary(byteArrayOf(7))):
...
private suspend fun caughtCancellation(block: suspend () -> Unit): CancellationException = try {
    block()
