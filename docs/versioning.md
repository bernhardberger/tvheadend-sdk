...
The source is configured for the provisional `0.21.0` release under these
coordinates:

- `at.bernhardberger.tvheadend:sdk-android:0.21.0`
- `at.bernhardberger.tvheadend:sdk-core:0.21.0`
- `at.bernhardberger.tvheadend:sdk-media3:0.21.0`
- `at.bernhardberger.tvheadend:sdk-playback:0.21.0`
- `at.bernhardberger.tvheadend:sdk-testing:0.21.0`

Source, local staging, and CI
do not establish that the configured `0.21.0` coordinates are publicly available;
...
version. Patch versions are reserved for backward-compatible fixes. The `0.21.0`
minor adds `MediaItem.isTvheadendLive()` and
`LivePlaybackObservation.Active.serverStopped`; `Active` gains
`component4()`, and its `equals`, `hashCode` and `toString` now include the new
property. The `0.20.0`
minor exposes live TVHeadend text subtitle streams as Media3 text tracks without
a public API change; applications that list or select text tracks see the new
tracks. The `0.19.0`
...
`PlaybackStopResult.PlayerUnavailable` value matches with type checks. The `0.17.0`
...
verified growing-recording seek bounds with a retained PCR probe and retires that
probe only after confirming the final extent. The `0.13.1`
...
presentation intent. See the changelog for the new command-stage outcomes.
The `0.12.1`
patch adds maintained finite paused IDR draining and clock precision repair without
changing public signatures. Use `createTvheadendRenderersFactory` for the decoder
integration; see [paused frame recovery](paused-frame-recovery.md) for limits. The
`0.12.0` minor adds opaque seek-correlation properties and changes the public
`TimeshiftTestFixture.playbackPosition` JVM signature. Recompile consumers; see
[content seek settlement](content-seek-settlement.md) for the changed behavior.
The preceding `0.11.0` minor adds nullable queue error diagnostics and changes the public
